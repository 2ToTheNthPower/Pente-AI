...
class MultiPente():
  def __init__(self, n_players=2, n_games=100, agent=None, graph_path=None):
    self.boards = np.zeros(shape=(n_games, 3, 3)).astype(int)
    self.prev_boards = np.zeros(shape=(n_games, 3, 3)).astype(int)
    ...
    if make_random_moves:
      ...
      while np.any(np.sum((np.amax(self.values, axis=(1,2), keepdims=True) == self.values), axis = (1,2)) != 1):
        ...
        if too_many_choices.shape[0] != 0:
          ...
          seq = np.vstack([rng.permutation(9).reshape(1,3,3) for i in range(too_many_choices.shape[0])]).astype("float64")
        
          too_many_choices += np.random.normal(0, .25, size=(too_many_choices.shape[0], 3, 3))
...
### 
  ...
  def play_train(self, agent0, agent1, make_random_moves=[False, False], num_games=100):
    ...
    count = 0
    player = -1
    while not np.sum(self.completed) == self.n_games and count < 8:
  ...
  def reset(self):
    ...
    self.boards = np.zeros(shape=(self.n_games, 3, 3))
...
class Agent():
  def __init__(self, agent_path = None, intelligence = 1):
    ...
    if agent_path is None:
      inp1 = Input(shape=(3,3,1))
      inp_mask = Input(shape=(3,3,1))

      # Convolve
      conv1 = Conv2D(64, 2, activation="relu")(inp1)
      norm1 = LayerNormalization()(conv1)

      deconv1 = Conv2DTranspose(64, 2, activation="relu")(norm1)
      norm2 = LayerNormalization()(deconv1)

      conv2 = Conv2D(64, 2, activation="relu")(norm2)
      norm3 = LayerNormalization()(conv2)

      deconv2 = Conv2DTranspose(1, 2, activation="relu")(norm3)
      ...
      self.model.compile(loss=lambda y_true, y_pred: 100000 * keras.losses.MeanAbsoluteError()(y_true, y_pred), optimizer="rmsprop")
  ...
  def fit_on_graph(self, graph, num_epochs = 25, batch_size = 256, sample_size = 1000):
    ...
    for epoch in range(num_epochs):
        ...
        if node in self.node_values.keys():
          ...
          for data in l:
            ...
            if data[0] not in self.node_values.keys():
              ...
              self.node_values[data[0]] = (np.zeros(shape=(3,3)), np.zeros(shape=(3,3)), np.zeros(shape=(3,3)))
      ...
      outcome_train = np.reshape(outcome_train, (outcome_train.shape[0], 3, 3, 1))
  ...
  def getValues(self, boards, pairs, player_num, graph, train=True, random_agent=False):
    ...
    if train:
      ...
      if not random_agent:
        ...
        child_count_array = np.array([self.node_values[node][2] for node in node_list]).reshape((n, 3, 3, 1))
        outcome_array = np.array([self.node_values[node][0] for node in node_list]).reshape((n, 3, 3, 1))

        policy_preds = self.model.predict([boards, pred_masks])

        ucb_score = np.array([outcome_array + self.c * (policy_preds / self.scale_factor) * np.sqrt(np.divide(np.log(parent_count_array + .0001), child_count_array + .000000001))]).reshape((n, 3, 3))
        ...
        return np.random.uniform(0,1,size=(n, 3, 3))
      ...
      if not random_agent:
        ...
        return policy_preds.reshape(n, 3, 3)
...
for batch in range(num_batches):
  ...
  agent0.fit_on_graph(multi.graph, num_epochs=3, batch_size=256, sample_size=1000)
